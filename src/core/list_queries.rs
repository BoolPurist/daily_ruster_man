...
pub fn fetch_all_monthly_names(_month_in_year: &FindByMonthInYear) -> AppResult<Vec<String>> {
    let with_valid_format: Vec<MonthlyName> = file_access::fetch_valid_date_entries()?;

    let filtered_monthlies =
        date_filtering::filter_monthly_by_ym(with_valid_format, _month_in_year);
